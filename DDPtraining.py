    ...
    if not eval_only:
        ...
        for i in range(1, n_epochs+1):
            ...
            if i % 100 == 0 or i == 1:
                ...
                print('***debug***: score len =', len(score))
                fid_details.loc[len(fid_details)] = score
                fid_details.to_csv(os.path.join(root, f'fid_details{rank}.csv'))
                FID_logger.concat(score[::7])
            if i % avg_mod == 0:
                average_params(model_G, 'G')
            if share_D:
                average_params(model_D, 'D')
            ...
            if i % 100 == 0 or i == 1:
                ...
                fid_details_averaged.loc[len(fid_details_averaged)] = score
                fid_details_averaged.to_csv(os.path.join(root, f'fid_details_averaged{rank}.csv'))
                FID_logger_averaged.concat(score[::7])
